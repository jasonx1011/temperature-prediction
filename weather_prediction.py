...
def linear_regr(X_train, y_train, X_test, y_test, poly_degree, interaction_only, print_coef, plot, ask_user, model_result):
    ...
    # for model in [0 1 2 3 4]:
        ...
        # print("Mean squared error (train): {0:.2f} \n".format( mse_train ) )
        # # Explained variance score: 1 is perfect prediction
        # print("Variance score (train): {0:.2f} \n".format( score_train ) )
        # print("model_runtime (train) = {0:.8f} (seconds) \n".format(model_runtime))
        # print("model_runtime (predict train) = {0:.8f} (seconds) \n".format(model_runtime_predict_train))
        # 
        # print("For test set:")
        ...
        # # The mean squared error
        # print("Mean squared error (test): {0:.2f} \n".format( mse_test ) )
        # # Explained variance score: 1 is perfect prediction
        # print("Variance score (test): {0:.2f} \n".format( score_test ) )
        # print("model_runtime (predict test) = {0:.8f} (seconds) \n".format(model_runtime_predict_test))
        # 
        # with open("logs/log_" + log_timestr +".txt", "a") as logfile:
        #     logfile.write("====================\n")
        ...
        #     logfile.write("Mean squared error (train): {0:.2f} \n".format( mse_train ) )
        #     logfile.write("Variance score (train): {0:.2f} \n".format( score_train ) )
        #     logfile.write("For test set: \n")
        #     logfile.write("Mean squared error (test): {0:.2f} \n".format( mse_test ) )
        #     logfile.write("Variance score (test): {0:.2f} \n".format( score_test ) )
        #     logfile.write("model_runtime (train) = {0:.8f} (seconds) \n".format(model_runtime))
        #     logfile.write("model_runtime (predict train) = {0:.8f} (seconds) \n".format(model_runtime_predict_train))
        #     logfile.write("model_runtime (predict test) = {0:.8f} (seconds) \n".format(model_runtime_predict_test))
        #     logfile.write("====================\n")
        # 
        # # collect info.
        # model_result[(model_name, poly_degree)] = (mse_train, score_train, mse_test, score_test, model_runtime, model_runtime_predict_train, model_runtime_predict_test)
        # 
        # # print shape
        # if (plot == True):
